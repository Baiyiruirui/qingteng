...
export function VerticalPoem({ poem }: { poem: Poem }) {
  return (
    <div className="flex flex-row-reverse items-start justify-center gap-6 select-none">
      ...
      {/* 题款列:标题 + 朝代·作者,单独成列,字号略小 */}
      <ol className="ml-2 flex flex-col items-center gap-2 list-none self-start pt-1">
        {poem.title.split("").map((ch, j) => (
          <li key={`t-${j}`} className="font-serif text-xl leading-none text-ink-mid">
            {ch}
          </li>
        ))}
        <li aria-hidden className="my-1 h-4 w-px bg-edge" />
        {`${poem.dynasty}·${poem.author}`.split("").map((ch, j) => (
          <li key={`a-${j}`} className="font-serif text-base leading-none text-ink-faint">
            {ch}
          </li>
        ))}
      </ol>
