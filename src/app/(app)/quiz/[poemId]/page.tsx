...
type SessionState =
  ...
  | { phase: 'quiz'; sessionId: string; questions: SafeQuestion[]; current: number; userAnswer: string; submitted: JudgeResult | null; results: boolean[] }
  | { phase: 'summary'; results: boolean[] }
...
export default function QuizPage() {
  ...
  function next() {
    ...
    const results = [...state.results, state.submitted.isCorrect]
    const nextIdx = state.current + 1
    if (nextIdx >= state.questions.length) {
      setState({ phase: 'summary', results })
    } else {
      setState({ ...state, current: nextIdx, userAnswer: '', submitted: null, results })
    }
  }
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-4" style={{ background: '#fafaf7' }}>
        <div className="w-full max-w-md rounded-2xl border p-8 text-center space-y-4" style={{ background: '#fff', borderColor: '#e8e4dc' }}>
          <div className="text-5xl font-serif">{pct >= 80 ? '🎉' : pct >= 60 ? '👍' : '💪'}</div>
          <h2 className="text-2xl font-serif">{correct} / {total}</h2>
          <p className="text-sm" style={{ color: '#6a6a6a' }}>
            {pct >= 80 ? '掌握得很好，继续保持！' : pct >= 60 ? '不错，还有提升空间～' : '多读几遍，你一定能行！'}
          </p>
        ...
        {submitted && (
          <div className="space-y-4">
            ...
            <div
              ...
              style={{
                background: submitted.isCorrect ? '#e8f4f0' : '#fdf0f0',
                color: submitted.isCorrect ? '#3d8c78' : '#c0392b',
              }}
            >
              <span className="text-xl">{submitted.isCorrect ? '✓' : '✗'}</span>
              <span className="font-medium">{submitted.isCorrect ? '答对了！' : '这次没答到～'}</span>
            </div>
            ...
            {/* Subjective: hit/miss points + feedback */}
            {!isObjective && (
              <div className="space-y-3">
                {submitted.hitPoints && submitted.hitPoints.length > 0 && (
                  <div className="rounded-xl border p-4 space-y-1.5" style={{ borderColor: '#d4ece5', background: '#f4fbf8' }}>
                    <p className="text-xs font-medium" style={{ color: '#3d8c78' }}>答到的得分点</p>
                ...
                {submitted.missedPoints && submitted.missedPoints.length > 0 && (
                  <div className="rounded-xl border p-4 space-y-1.5" style={{ borderColor: '#f0d9d9', background: '#fdf5f5' }}>
                    <p className="text-xs font-medium" style={{ color: '#c0392b' }}>未答到的得分点</p>
                    {submitted.missedPoints.map((pt, i) => (
                      <p key={i} className="text-sm" style={{ color: '#922b21' }}>✗ {pt}</p>
                    ))}
                  </div>
                )}
                {submitted.feedback && (
