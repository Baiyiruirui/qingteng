...
| Phase E-11 | ✅ | 服务端权威聊天历史、轮次幂等与恢复队列、生成预算和中断传播 |
| Phase E-12 | ✅ | Memory 查看、纠正、删除、暂停与保留期限，策略校验 21/21 |
| Phase E-13 | ✅ | 腾讯标准示范音、云端拼音、逐句跟读和移动端朗读练习台 |
| Phase E | ⏳ | 3 分钟视频、README 终稿、密钥轮换、最终归档 |

最近关键提交：

```text
cba9b94 feat: refine guided recitation and memory controls
100c251 feat: add guided recitation with AI standard reading
c584f0a feat: add user-controlled memory management
8219ea3 feat: bound chat generation and propagate aborts
0dafd63 fix: make chat turns idempotent and recoverable
04accd9 fix: rebuild chat history from server state
...
- `pnpm verify:security` 当前 31/31 checks；唯一 warning 是 `QT_ADMIN_USER_IDS` 未配置，因此生产内部工具默认全部拒绝。
...
- 普通聊天和沉浸聊天只接受本轮消息，可信历史由服务端从数据库重建；`clientMessageId`、Redis 轮次锁、完成结果缓存和恢复队列共同提供幂等与失败恢复。
- 模型调用统一输出 token 上限、硬超时与请求中断传播；重放请求不会重复消耗模型额度。
- 用户可在“我的”页面查看、纠正、删除或清空长期 Memory，并暂停后续记忆、设置 30/90/180 天或永久保留期限。
- 朗读页已支持腾讯标准示范音、完整云端拼音、逐句/整首模式和字句对齐反馈；`pnpm test:recite` 为 19/19。
...
- UI 收口验证：`pnpm build` 通过，`verify:security` 31/31（1 个 warning），`verify:wrong-question` 5/5；个人 Memory 和朗读页已完成桌面/390px 移动截图复验。
...
1. 按安全清单轮换腾讯云和 Langfuse 密钥，再更新 Vercel 并复验聊天、trace、示范音和朗读。
2. 配置专用 `E2E_USER_NAME` / `E2E_USER_PASSWORD`，补跑 2 条登录后主链路。
3. 补 Demo 视频脚本，使用真实口径“140 首基础内容 + 14 首/95 道深题库”。
