...
- **当前状态**:后半程按 CHARTER.md 执行。Phase A 的 P0 demo 落地缺口已收口,准备进入 Phase B Eval + Langfuse
...
- AI:Vercel AI SDK + DeepSeek + Claude Haiku(后续)+ Whisper(后续);出题用 generateObject + Zod 结构化输出 + grounding(poem 数据注入 + evidenceLines 溯源校验)
...
| Phase B | 🔨 | Eval v0 runner + golden-v0 已落地;judge 后处理回归 22/22;Langfuse 已接入 chat/opening/quiz-generate/quiz-judge/memory-extract;50 题扩容待做 | - |
