...
| Phase E-9 | ✅ | 今日案头/对话双向切换 + `/chat` 默认回案头；诗笺朝代显示归一化，`Tang` 兜底为“唐” | `bea335d` |
| Phase E-10 | ✅ | 今日案头可读性与交互收口：垂藤弱化、双学习入口、悬浮对话栏、IME/移动端输入保护 | `b6f7796` / `084da03` |
| Phase E-11 | ✅ | 对话可靠性加固：服务端权威历史、轮次幂等锁与恢复队列、输出预算和请求中断传播 | `04accd9` / `0dafd63` / `8219ea3` |
| Phase E-12 | ✅ | 用户可控 Memory：查看、纠正、删除、暂停记忆与保留期限，策略校验 21/21 | `c584f0a` |
| Phase E-13 | ✅ | 儿童友好朗读：腾讯标准示范音、云端拼音、逐句跟读与移动端练习台 | `100c251` / `cba9b94` |
